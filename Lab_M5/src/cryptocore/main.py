...
def _run_hmac(args: object) -> int:
    input_path = args.input
    input_label = str(input_path)
    try:
        if input_label == "-":
            value = hmac_stream(sys.stdin.buffer, args.key)
        else:
            value = hmac_file(input_path, args.key)

        if args.verify is not None:
            expected = read_expected_hmac(args.verify)
            if constant_time_equal(bytes.fromhex(value), expected):
                print("[OK] HMAC verification successful")
                return 0
            print("[ERROR] HMAC verification failed", file=sys.stderr)
            return 1

        line = f"{value} {input_label}"
        if args.output is None:
            print(line)
        else:
            write_digest_line(args.output, line)
    except (HashOperationError, MacOperationError, OSError, ValueError) as error:
        print(f"Ошибка: {error}", file=sys.stderr)
        return 1
    return 0
