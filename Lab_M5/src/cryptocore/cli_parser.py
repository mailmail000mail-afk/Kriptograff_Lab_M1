...
def parse_hex_mac_key(value: str) -> bytes:
    if len(value) % 2 != 0 or any(symbol not in string.hexdigits for symbol in value):
        raise argparse.ArgumentTypeError(
            "ключ HMAC должен быть шестнадцатеричной строкой чётной длины"
        )
...
def build_digest_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="ключ HMAC произвольной длины в hex",
    )
    parser.add_argument(
        "--verify",
        type=Path,
        help="сравнить HMAC с первым значением из указанного файла",
    )
    return parser
    ...
    if raw_arguments and raw_arguments[0] == "dgst":
        ...
        if args.hmac:
            ...
            if args.key is not None:
                parser.error("--key допускается только вместе с --hmac")
            if args.verify is not None:
                parser.error("--verify допускается только вместе с --hmac")
        args.command = "dgst"
        return args
